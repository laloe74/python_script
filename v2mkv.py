...
def write_summary(expected_count, success_count, fail_count, total_time, unsupported_files, failed_files, success_files, no_processing_files):
    with open(log_file_path, 'a') as log_file:
        log_file.write("\n## 基本信息\n")
    ...
    write_log_section("错误信息", [f"不支持的文件：{file}" for file in unsupported_files])
    write_log_section("错误信息", [f"未处理的文件：{file}" for file in failed_files])
    write_log_section("成功信息", success_files)
    write_log_section("无须处理", no_processing_files)
    ...
    for root, dirs, files in os.walk(from_folder):
        for file in files:
            ...
            if file_extension in supported_formats:
                        ...
                        if "time=" in line:
                            ...
                            if time_pattern.match(time_str):
                                ...
                                if duration:
                                    ...
                                    print(f"\r正在处理: {processed_files}/{total_files}/{error_files} {int(progress)}% {progress_bar} -- {file} ({elapsed_time:.2f}s)", end='')
                    
                    process.wait()
                    if process.returncode == 0:
                        success_files.append(file)
                        write_log_section("成功信息", [file])
                        success_count += 1
                    else:
                        failed_files_list.append(file)
                        fail_count += 1
                        print(f"\n转换失败: {input_path}")
                        write_log_section("错误信息", [f"未处理的文件：{file}"])
                ...
                write_log_section("错误信息", [f"不支持的文件：{file}"])

    end_time = time.time()
    total_time = end_time - start_time
    write_summary(total_files, success_count, fail_count, total_time, unsupported_files, failed_files_list, success_files, no_processing_files)
